...
client.on("message", (msg) => {
  ...
  //инфобот
  if (msg.content === 'инфобот баланс'){
    infobot_getBalance().then(data => {
      msg.reply(`${rp(['Баланас сервиса инфобот на текущий момент', 'Мой ответ', 'Где-то в районе'])} ${data} руб.`)
    })
  }
});
...
cron.schedule("*/30 * * * *", () => {
  sms.my_balance((e) => {
    ...
    hde().then(res => { //<@&846295813617221632> тег роли
    client.channels.cache
    .get("844589763935207446")
    .send(`${rp(['Количество не закрытых тикетов', 'Не закрытые тикеты', 'Тикетов не закрыто', 'Тикеты к закрытию'])} - ${res.pagination.total}.
    ${rp(['да прибудет с вами сила', 'пора бы их все позакрывать, не думаете?', 'ну как ж так вышло-то?'])}`);
    })
    infobot_getBalance().then(data => { 
      client.channels.cache
        .get("844589763935207446")
